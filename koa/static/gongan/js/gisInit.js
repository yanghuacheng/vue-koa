...
function addSJZCModel() {
    ...
    function loadDimian() {
        ...
        dimian = viewer.scene.primitives.add(new Cesium.Cesium3DTileset({
            url: "./static/sjzc/sjzc/tileset.json",
        ...
        // dimian.style = new Cesium.Cesium3DTileStyle(style);
    ...
    function addworld00() {
        // world00
        let world = viewer.scene.primitives.add(new Cesium.Cesium3DTileset({
            url: "./static/world00/tileset.json",
            maximumNumberOfLoadedTiles: 100000
        }));
        //贴地显示
        var height2 = -3; //-3.77
        world.readyPromise.then(function (argument) {
            var cartographic = Cesium.Cartographic.fromCartesian(world.boundingSphere.center);
            // console.log(cartographic);
            var surface = Cesium.Cartesian3.fromRadians(cartographic.longitude, cartographic.latitude, cartographic.height);
            var offset = Cesium.Cartesian3.fromRadians(cartographic.longitude, cartographic.latitude, cartographic.height +
                height2);
            var translation = Cesium.Cartesian3.subtract(offset, surface, new Cesium.Cartesian3());
            world.modelMatrix = Cesium.Matrix4.fromTranslation(translation);
        })
    }
    // addworld00();
...
function zuizhongShow(zhongPoints) {
    if(zhongPoints.length > 0){
        let obj = zhongPoints[0]
        viewer.camera.flyTo({
            destination: Cesium.Cartesian3.fromDegrees(obj.longitude, obj.latitude, 150),
            orientation: {
                heading: 0,
    ...
    zhongPoints.forEach(function (item, indx) {
        ...
        xuxianPoints.push(zhongPoints[indx].latitude);
        xuxianPoints.push(zhongPoints[indx].height + 20);
        // xakj.customLabelPoints(113.970038254,22.538064471, 10, { count: 12, address: "北环科苑天桥",img:'./images/xianyir.png' });
        xakj.customLabelPoints(zhongPoints[indx].longitude, zhongPoints[indx].latitude, zhongPoints[indx].height + 10, { count: zhongPoints[indx].count, address: zhongPoints[indx].address, img: jiansuoimg });
        ...
        if (indx > 0) {
            let _data = {
                ...
                options: {
                    ...
                    polyline: {
                        width: 5,//线宽度
                        material: [Cesium.Color.RED, 7000],//混合颜色、(红绿混合透明后 就是黄色了)3000秒发射间隔,单纯材质无法展示飞行动态。所以去掉了。
                    }
                }
            };
            //加载流动线
    ...
    if (currentSelectedEntity) {
        if (currentSelectedEntity.deviceType == "camera") {
            ...
            if (currentSelectedEntity.status > 0) {
                ...
                plar = playVideo("rtmp://190.32.33.80:9935/live/" + currentSelectedEntity.code_id);
            }
        }
        //点击全景
...
function ljfunc(obj) {
    ...
    //先移除上一个脉冲
    if(xakj.tuisongMaiChong.length >0){
        xakj.tuisongMaiChong.forEach(function(item){
            if(item.name == 'tuisong'){
                console.log("清除")
                viewer.entities.remove(item);
            }
        })
...
$(document).ready(function () {
    var _s = false;
    var _div = null;
    // $('.zrdw').on('click', function () {
    //     // debugger
    //     // if ($(this).index() == 7) {
    //       if($('.jingli').is(':visible')){
    //         // if (!_s) {
    //         //     if(devMap.containsKey("jingl")){
    //         //         _div = devMap.get("jingl")
    //         //     }
    //         //     if (!_div) _div = addJingLi();
    //         //     xakj.universalLabelPoint(113.97085904, 22.538727841, 3, _div);
    //         //     $('.jingli').show();
    //             viewer.camera.flyTo({
    //                 destination: Cesium.Cartesian3.fromDegrees(113.97085904, 22.538727841, 130),
    //                 orientation: {
    //                     heading: 0,
    //                     pitch: Cesium.Math.toRadians(-90),
...
function addJingLi() {
    ...
    $(_div).html(
        '<div>' +
        '<span class="jingli-adr">世界之窗交警大队</span>' +
        '<span class="jingli-man">25<span>人</span></span>' +
        '</div>'
    );
    devMap.put("jingl", _div);
    return _div;

}

//测试添加警力
function testAddJingliPoint(){
    // 传过去data
    let  data = {
            title:"世纪之窗警力666", //标题
            policeForce:'25' //人数
        ...
        let type = 1;
    xakj.addJingliPoint(type,data,function(lon,lat,height,div){
        //返回坐标
        let divArr = []
        xakj.universalLabelPoint(lon, lat, height, div);
        if(devMap.containsKey("jingliArr")){
            divArr = devMap.get(jingliArr);
        }
        divArr.push({
            div:div,
            position:{
