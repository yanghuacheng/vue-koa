...
function MeasureTools(viewer) {
     ...
     * 测距
     */
    this.measurePolyLine = function (callback) {
            ...
            if (positions.length == 0) {
                ...
                viewer.screenSpaceEventHandler.setInputAction(function (moveEvent) {
                    ...
                    if (positions.length == 2) {
                        ...
                        // 计算距离
                        cb = getLengthText(positions[0], positions[1])
                        callback(cb)
                        var lengthText = "距离：" + cb;
     ...
     * 测面积
     */
    this.measurePolygon = function (callback) {
            ...
            if (positions.length == 0) {
                ...
                viewer.screenSpaceEventHandler.setInputAction(function (moveEvent) {
                    ...
                    if (positions.length >= 3) {
                        ...
                        let mj = getArea(positions)
                        callback(mj)
                        var text = "面积：" + mj;
                        
                        
                        // var centerPoint = getCenterOfGravityPoint(positions);
                        // console.log("pppsd得到",positions);
                        var centerPoint = Cesium.BoundingSphere.fromPoints(positions).center
                       
                        var cartographic = Cesium.Cartographic.fromCartesian(centerPoint);
                        var lng = Cesium.Math.toDegrees(cartographic.longitude);
                        var lat = Cesium.Math.toDegrees(cartographic.latitude);
                        var height = cartographic.height;//模型高度
                        // console.log("pppsd得到",height);
                        labelEntity = addLabel(Cesium.Cartesian3.fromDegrees(lng,lat,height + 13), text);
     ...
     * 测高
     */
    this.measureHeight = function (callback) {
        var positions = [];
            ...
            if (positions.length == 0) {
                ...
                viewer.screenSpaceEventHandler.setInputAction(function (moveEvent) {
                    ...
                    if (positions.length >= 2) {
                        ...
                        // 计算距离
                       let  s_p = getLengthText(positions[0], positions[1]);
                        var lengthText_1 = "水平距离：" + s_p;
                        ...
                        // 计算距离
                        let c_z = getLengthText(positions[1], positions[2])
                        var lengthText_2 = "垂直距离：" + c_z;
                        ...
                        // 计算距离
                        let z_x = getLengthText(positions[2], positions[3])
                        var lengthText_3 = "直线距离：" + z_x;
