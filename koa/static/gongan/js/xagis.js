...
function xakjGis(container, data) {
    ...
    // data["showRenderLoopErrors"] = false;//是否显示报错界面
    data["shadows"] = false //确定阴影是否由太阳投射
    data["skyAtmosphere"] = false
    ...
    viewer.scene.globe.enableLighting = true;//开启全照明
...
xakjGis.prototype.addCircleRipple = function (data) {
    ...
    function remove() {
        //先移除
        if (self.maiChongArr.length > 0) {
            self.maiChongArr.forEach(function (item) {
 ...
 * 偏移量
 */
xakjGis.prototype.customLabelPoints = function (lng, lat, height, data) {

    console.log("自定义标注");
    //创建html
    let zz_div = document.createElement('div');
    zz_div.style.display = 'block';
    zz_div.className = 'zz_point2';
    let zz_adress = document.createElement('div');
    zz_adress.className = 'zz_point2_adress';
    let zz_number = document.createElement('div');
    zz_number.className = 'zz_point2_number';
    let zz_img = document.createElement('img');
    let zz_span = document.createElement('span');
    zz_div.appendChild(zz_adress);
    zz_adress.appendChild(zz_img);
    zz_div.appendChild(zz_number);
    zz_number.appendChild(zz_span);
    ...
    document.getElementById('main').appendChild(zz_div);
    var ellipsoid = viewer.scene.globe.ellipsoid;
    ...
    function changePopup(c) {
        ...
        zz_div.style.left = c.x - 27 + 'px';
        zz_div.style.top = c.y - 37 + 'px';
    }
 ...
 * 传入动态创建的div
 */
xakjGis.prototype.universalLabelPoint = function (lng, lat, height, div, left, top) {
    let destination = Cesium.Cartesian3.fromDegrees(lng, lat, height);
    document.getElementById('main').appendChild(div);
    var ellipsoid = viewer.scene.globe.ellipsoid;
    var cartographic = Cesium.Cartographic.fromDegrees(lng, lat, height);
    ...
    function changePopup(c) {
        ...
        div.style.left = c.x - 245 + 'px';
        div.style.top = c.y - 256 + 'px';
    }
    let c = new Cesium.Cartesian2(obj.position.x, obj.position.y);
    changePopup(c);
...
//主体责任单位 警力标注
xakjGis.prototype.addJingliPoint = function(type,data,callback){
    let self = this
    // data = {
    //     title:"世纪之窗警力", //标题
    //     policeForce:'25' //人数
   ...
   let handler = new Cesium.ScreenSpaceEventHandler(viewer.scene.canvas);
    handler.setInputAction(function(movement) {
        ...
        $('.addPoint').show();
        $('.addPoint').css('left',movement.endPosition.x + 10 +"px");
        $('.addPoint').css('top',movement.endPosition.y +"px");
       ...
       if(!_div){
        ...
        if(type==0){
            $(_div).html(
                '<div>' +
                '<span class="jingli-adr">'+ data.title+'</span>' +
                '</div>'
              );
        }else{
            $(_div).html(
                '<div>' +
                '<span class="jingli-adr">'+ data.title+'</span>' +
                '<span class="jingli-man">'+ data.policeForce +'<span>人</span></span>' +
                '</div>'
              );
        }
        ...
        // $(_div).show();
        $(_div).css({opacity:1});
        $(_div).animate(function(){
            
        },500,function(){
            $(_div).fadeIn(500)
        })
       } 
        $(_div).css('left',movement.endPosition.x - 177 + 'px');
        $(_div).css('top',movement.endPosition.y - 170 +"px");
        let mp = movement.endPosition;
        // mp.y = mp.y + 170;
        // mp.x = mp.x + 177;
        var cartesian = viewer.camera.pickEllipsoid(mp, viewer.scene.globe.ellipsoid);
        if (cartesian) {
            ...
            height = Cesium.Math.toDegrees(cartographic.height).toFixed(10);
        }
    ...
    handler.setInputAction(function(movement){
        ...
        callback({lon:lon,lat:lat,height:height,_div:_div});
        handler.destroy()
        $('.addPoint').hide();
        self.universalLabelPoint(lon,lat,height,_div);
